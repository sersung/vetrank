import { and, asc, desc, eq, gte, inArray, isNotNull, ilike, lte, or, sql } from "drizzle-orm";
...
export async function upsertUser(user: InsertUser): Promise<void> {
  ...
  // Admin role is set in sdk.ts when the user's email matches OWNER_EMAIL
  if (user.role === "admin") {
    values.role = "admin";
  ...
  // Set trial plan for new users (only on INSERT, not on conflict update)
...
export async function addXp(
  ...
  await db
    ...
    .onConflictDoUpdate({ target: [weeklyXp.userId, weeklyXp.weekKey], set: { xp: sql`${weeklyXp.xp} + ${amount}` } });

  await db
    .insert(monthlyXp)
    .values({ userId, monthKey, xp: amount })
    .onConflictDoUpdate({ target: [monthlyXp.userId, monthlyXp.monthKey], set: { xp: sql`${monthlyXp.xp} + ${amount}` } });
...
export async function getQuestions(filters: {
  ...
  if (filters.search) conditions.push(ilike(questions.textPt, `%${filters.search}%`));
...
export async function getDiscursiveQuestions(filters: {
  ...
  if (filters.search) conditions.push(ilike(discursiveQuestions.textPt, `%${filters.search}%`));
...
export async function seedBadges(): Promise<void> {
  ...
  for (const badge of badgeData) {
    await db.insert(badges).values(badge).onConflictDoUpdate({ target: badges.slug, set: { namePt: badge.namePt } });
  }
}
...
export async function seedDisciplines(): Promise<void> {
  ...
  for (const d of disciplineData) {
    await db.insert(disciplines).values(d).onConflictDoUpdate({ target: disciplines.slug, set: { namePt: d.namePt } });
  }
}
